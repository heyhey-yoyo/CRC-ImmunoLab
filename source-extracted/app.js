...
(function (CRC3) {
    class Charts {
        ...
        static heatmap(canvas, result) {
            ...
            matrix.forEach((row, iy) => row.forEach((v, ix) => { if (v === null) { ctx.fillStyle = '#12242b'; ctx.fillRect(pad.l + ix * cw, pad.t + iy * ch, cw - 2 * dpr, ch - 2 * dpr); ctx.fillStyle = '#66868e'; ctx.font = `${9 * dpr}px sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('N/A', pad.l + (ix + .5) * cw, pad.t + (iy + .5) * ch); return; } const t = (v - lo) / (hi - lo || 1); const r = Math.round(38 + 190 * t), g = Math.round(199 - 105 * t), b = Math.round(173 - 60 * t); ctx.fillStyle = `rgb(${r},${g},${b})`; ctx.fillRect(pad.l + ix * cw, pad.t + iy * ch, cw - 2 * dpr, ch - 2 * dpr); ctx.fillStyle = t > .55 ? '#fff' : '#061615'; ctx.font = `${9 * dpr}px sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(v.toFixed(2), pad.l + (ix + .5) * cw, pad.t + (iy + .5) * ch); }));
