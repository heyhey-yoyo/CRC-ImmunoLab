...
(function (CRC3) {
    CRC3.VERSION = '1.0.1';
    CRC3.BATCH_MODEL_VERSION = '1.0.1';
    CRC3.SPATIAL_MODEL_VERSION = '1.0.0';
    CRC3.ARM_COLORS = {
        control: '#5f7d7c', chemo: '#b08050', pd1: '#6b8cae', combo: '#c15f3c', triple: '#a2678e'
    };
...
(function (CRC3) {
    ...
    function normalizeStudyResult(value) {
        ...
        return {
            ...
            completedAt: text(raw.completedAt, new Date().toISOString(), 40), modelVersion: text(raw.modelVersion, 'unknown', 20), applicationVersion: text(raw.applicationVersion, 'unknown', 20), configFingerprint: CRC3.fingerprint(config), recordedConfigFingerprint: text(raw.recordedConfigFingerprint || raw.configFingerprint, '', 32), config, runs,
    ...
    function normalizeProjectPayload(value) {
        ...
        if (payload.config && payload.runs && payload.aggregates && !payload.studies) {
            ...
            return { id: CRC3.uid('project'), name: `${study.name} 项目`, description: '从单项研究结果导入', createdAt: now, updatedAt: now, modelVersion: CRC3.BATCH_MODEL_VERSION, config: study.config, studies: [study] };
        }
        if (payload.preset && payload.arms && !payload.config) {
            const now = new Date().toISOString(), config = normalizeStudyConfig(payload);
            return { id: CRC3.uid('project'), name: text(payload.name, '导入研究项目', 80), description: '从研究配置创建', createdAt: now, updatedAt: now, modelVersion: CRC3.BATCH_MODEL_VERSION, config, studies: [] };
        }
        ...
        return {
            ...
            createdAt: text(payload.createdAt, now, 40), updatedAt: now, modelVersion: text(payload.modelVersion, 'unknown', 20),
...
(function (CRC3) {
    class Charts {
        static resize(canvas) {
            ...
            this.ink = style.getPropertyValue('--muted').trim() || '#5f5a53';
            this.lineColor = style.getPropertyValue('--line').trim() || '#d5cbbb';
        ...
        static line(canvas, aggregates, metric = 'tumor') {
            ...
            if (!Number.isFinite(xmax) || xmax <= 0) {
                this.empty(ctx, w, h, dpr, '时间序列无有效天数');
                return;
            }
            ...
            ctx.lineWidth = 1 * dpr;
            ctx.font = `${12 * dpr}px ${this.fontFamily}`;
        ...
        static distribution(canvas, aggregates) {
            ...
            aggregates.forEach((a, i) => { const cx = pad.l + slot * (i + .5), boxW = Math.min(32 * dpr, slot * .42); ctx.strokeStyle = a.color; ctx.fillStyle = a.color + '30'; ctx.lineWidth = 1.5 * dpr; ctx.beginPath(); ctx.moveTo(cx, y(a.q05)); ctx.lineTo(cx, y(a.q95)); ctx.stroke(); ctx.fillRect(cx - boxW / 2, y(a.q75), boxW, y(a.q25) - y(a.q75)); ctx.strokeRect(cx - boxW / 2, y(a.q75), boxW, y(a.q25) - y(a.q75)); ctx.beginPath(); ctx.moveTo(cx - boxW / 2, y(a.median)); ctx.lineTo(cx + boxW / 2, y(a.median)); ctx.stroke(); ctx.fillStyle = this.ink; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText(a.armName.length > 8 ? a.armName.slice(0, 7) + '…' : a.armName, cx, h - pad.b + 10 * dpr); });
        }
        static heatmap(canvas, result) {
            const { ctx, w, h, dpr } = this.resize(canvas);
            ctx.clearRect(0, 0, w, h);
            if (!result || !result.config.scan.enabled) {
                this.empty(ctx, w, h, dpr, '启用参数扫描后显示响应面');
                return;
            }
            ...
            if (!values.length || !arms.length) {
                this.empty(ctx, w, h, dpr, '暂无扫描结果');
                return;
            }
            const pad = { l: 70 * dpr, r: 20 * dpr, t: 20 * dpr, b: 42 * dpr };
            const cw = (w - pad.l - pad.r) / values.length, ch = (h - pad.t - pad.b) / arms.length;
            const all = [];
            const matrix = arms.map(arm => values.map(v => { const endpoints = result.runs.filter(r => r.armId === arm && r.scanValue === v).map(r => r.endpointTumor); if (!endpoints.length)
                return null; const median = CRC3.quantile(endpoints, .5); all.push(median); return median; }));
            if (!all.length) {
                this.empty(ctx, w, h, dpr, '暂无扫描结果');
                return;
            }
            const lo = Math.min(...all), hi = Math.max(...all);
            matrix.forEach((row, iy) => row.forEach((v, ix) => { if (v === null) { ctx.fillStyle = '#e8e1d6'; ctx.fillRect(pad.l + ix * cw, pad.t + iy * ch, cw - 2 * dpr, ch - 2 * dpr); ctx.fillStyle = this.ink; ctx.font = `${12 * dpr}px ${this.fontFamily}`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('N/A', pad.l + (ix + .5) * cw, pad.t + (iy + .5) * ch); return; } const t = (v - lo) / (hi - lo || 1); const r = Math.round(246 - 98 * t), g = Math.round(226 - 163 * t), b = Math.round(194 - 154 * t); ctx.fillStyle = `rgb(${r},${g},${b})`; ctx.fillRect(pad.l + ix * cw, pad.t + iy * ch, cw - 2 * dpr, ch - 2 * dpr); const linear = [r, g, b].map(v => v / 255).map(v => v <= .04045 ? v / 12.92 : ((v + .055) / 1.055) ** 2.4); const luminance = .2126 * linear[0] + .7152 * linear[1] + .0722 * linear[2]; ctx.fillStyle = luminance > .179 ? '#000' : '#fff'; ctx.font = `${12 * dpr}px ${this.fontFamily}`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(v.toFixed(2), pad.l + (ix + .5) * cw, pad.t + (iy + .5) * ch); }));
            ctx.fillStyle = this.ink;
            ctx.font = `${12 * dpr}px ${this.fontFamily}`;
            ctx.textBaseline = 'middle';
            ctx.textAlign = 'right';
            arms.forEach((a, i) => { const name = result.aggregates.find(x => x.armId === a)?.armName || a; ctx.fillText(name.length > 5 ? name.slice(0, 4) + '…' : name, pad.l - 8 * dpr, pad.t + (i + .5) * ch); });
            ctx.textAlign = 'center';
            ctx.textBaseline = 'top';
            values.forEach((v, i) => ctx.fillText(String(v.toFixed(0)), pad.l + (i + .5) * cw, h - pad.b + 8 * dpr));
        }
        static empty(ctx, w, h, dpr, text) { ctx.fillStyle = this.ink; ctx.font = `${12 * dpr}px ${this.fontFamily}`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(text, w / 2, h / 2); }
    }
    CRC3.Charts = Charts;
})(CRC3 || (CRC3 = {}));
var CRC3;
(function (CRC3) {
    const CELL_COLORS = ['#ff8282', '#5be0ce', '#8ab8ff', '#e9cf70', '#ffc069', '#c19aff', '#6c767b'];
    CRC3.cellColors = Object.freeze(CELL_COLORS);
    class SpatialWorkbench {
        ...
        createWorker() {
            const embedded = window.__CRC3_SPATIAL_WORKER_SOURCE__;
            if (!embedded)
                throw new Error('缺少内嵌 spatial.worker.js 源码，无法创建空间计算线程');
        ...
        size() { const dpr = Math.min(2, devicePixelRatio || 1), rect = this.canvas.getBoundingClientRect(), w = Math.max(1, Math.round(rect.width * dpr)), h = Math.max(1, Math.round(rect.height * dpr)); if (this.canvas.width !== w || this.canvas.height !== h) {
            this.canvas.width = w;
        ...
        render() {
            ...
            if (!snapshot) {
                ctx.fillStyle = '#5f7d7c';
                ctx.font = `${12 * dpr}px ${getComputedStyle(this.canvas).fontFamily}`;
                ctx.textAlign = 'center';
            ...
            ctx.font = `600 ${12 * dpr}px ${getComputedStyle(this.canvas).fontFamily}`;
            ctx.textAlign = 'left';
...
(function (CRC3) {
    class App {
        constructor(root) {
            ...
            this.startupWarning = '';
            this.analysisResizeFrame = null;
            window.addEventListener('resize', () => {
                if (this.view !== 'analysis') return;
                cancelAnimationFrame(this.analysisResizeFrame);
                this.analysisResizeFrame = requestAnimationFrame(() => this.drawAnalysisCharts());
            });
            this.root = root;
        }
        async init() {
        ...
        renderProjects() {
            ...
            el.innerHTML = `<div class="page-head"><div><h2>项目与研究记录</h2><p>本地优先的项目容器，保存配置、重复运行、统计摘要和空间快照引用。</p></div><div class="page-actions"><button class="quiet-btn" id="renameProjectBtn">编辑项目</button><button class="primary-btn" id="newStudyBtn">新建研究</button></div></div><div class="grid-3 mb-14"><div class="panel kpi-card"><div class="kpi-label">已完成研究</div><div class="kpi-value">${studies.length}</div><div class="kpi-sub">项目内批量研究记录</div></div><div class="panel kpi-card"><div class="kpi-label">累计随机运行</div><div class="kpi-value">${studies.reduce((n, s) => n + s.runs.length, 0)}</div><div class="kpi-sub">含参数扫描运行</div></div><div class="panel kpi-card"><div class="kpi-label">本地数据层</div><div class="kpi-value" style="font-size:21px">${this.storageMode === 'opfs' ? 'OPFS' : this.storageMode === 'memory' ? 'Memory' : 'LocalStorage'}</div><div class="kpi-sub">无需上传研究数据</div></div></div><div class="project-grid">${studies.length ? studies.map(s => this.studyTile(s)).join('') : `<div class="panel empty-state" style="grid-column:1/-1"><div><div class="empty-icon">⌁</div><h3>还没有完成的批量研究</h3><p>完成研究设计后进入运行中心。每个治疗臂将按随机种子重复运行，并生成分位区间与响应概率。</p><button class="primary-btn" id="emptyDesignBtn">开始设计研究</button></div></div>`}</div>`;
        ...
        bindDesign() {
            ...
            const restored = document.querySelector(`.arm-input[data-arm="${input.dataset.arm}"][data-key="${key}"]`);
            if (restored) {
                restored.focus();
                if (caret !== null)
                    try {
                    ...
                    catch (error) { /* number 类型输入不支持选区定位 */ }
            } });
        ...
        createBatchWorker() { const embedded = window.__CRC3_BATCH_WORKER_SOURCE__; if (!embedded)
            throw new Error('缺少内嵌 batch.worker.js 源码，无法创建批量计算线程');
        ...
        startStudy() { if (this.runStatus === 'running')
            ...
            this.renderRuns(); this.toast('运行失败', this.runError, true); }; this.batchWorker.postMessage({ type: 'RUN_STUDY', studyId, createdAt, applicationVersion: CRC3.VERSION, configFingerprint: CRC3.fingerprint(cfg), config: cfg }); }
        ...
        renderAnalysis() {
            ...
            requestAnimationFrame(() => this.drawAnalysisCharts());
        }
        drawAnalysisCharts() {
            if (this.view !== 'analysis') return;
            const study = this.activeStudy();
            if (!study || !document.getElementById('trajectoryChart')) return;
            CRC3.Charts.line(this.byId('trajectoryChart'), study.aggregates, this.analysisMetric);
            CRC3.Charts.distribution(this.byId('distributionChart'), study.aggregates);
            CRC3.Charts.heatmap(this.byId('heatmapChart'), study);
        }
        ...
        renderProvenance() { const el = this.byId('view-provenance'); el.innerHTML = `<div class="page-head"><div><h2>模型、参数证据与溯源</h2><p>明确区分文献支持、校准值与机制假设，并记录版本、范围和敏感性。</p></div><div class="page-actions"><button class="quiet-btn" id="exportModelCardBtn">导出模型卡</button><button class="primary-btn" id="exportProtocolBtn">导出研究协议</button></div></div><div class="grid-3 mb-14"><div class="panel kpi-card"><div class="kpi-label">当前配置指纹</div><div class="kpi-value mono" style="font-size:23px">${CRC3.fingerprint(this.project.config)}</div><div class="kpi-sub">配置变更会生成新指纹</div></div><div class="panel kpi-card"><div class="kpi-label">参数证据覆盖</div><div class="kpi-value">${CRC3.PARAMETER_EVIDENCE.length}</div><div class="kpi-sub">个已登记核心参数</div></div><div class="panel kpi-card"><div class="kpi-label">计算架构</div><div class="kpi-value" style="font-size:19px">Batch + ABM</div><div class="kpi-sub">批量随机 + 空间代理模型</div></div></div><div class="grid-2"><section class="panel"><div class="panel-head"><div><h3>参数证据库</h3><p>Parameter registry</p></div></div><div style="overflow:auto"><table class="data-table evidence-table"><thead><tr><th>参数</th><th>含义</th><th>默认/范围</th><th>证据类型</th><th>置信</th><th>敏感性</th></tr></thead><tbody>${CRC3.PARAMETER_EVIDENCE.map(p => `<tr><td><strong>${p.label}</strong><br><span class="mono text-muted">${p.id}</span></td><td>${p.meaning}</td><td>${p.defaultValue}<br><span class="text-muted">${p.range} · ${p.unit}</span></td><td><span class="evidence-level ${p.sourceType}">${p.sourceType === 'literature' ? '文献' : p.sourceType === 'calibrated' ? '模型校准' : '机制假设'}</span></td><td>${p.confidence}</td><td>${p.sensitivity}</td></tr>`).join('')}</tbody></table></div></section><div><section class="panel"><div class="panel-head"><div><h3>模型结构</h3><p>Two-tier compute architecture</p></div></div><div class="panel-body diagnostic-list"><div class="diagnostic-item"><div class="diag-icon">1</div><div><strong>批量随机机制模型</strong><p>用于 8–100 次重复、治疗臂比较、5%–95% 分位区间和参数扫描。状态变量以 0.25 天步长更新。</p></div></div><div class="diagnostic-item"><div class="diag-icon">2</div><div><strong>代表性空间代理模型</strong><p>用于细胞级迁移、接触杀伤、扩散场、组织切面和区域空间分析。</p></div></div><div class="diagnostic-item"><div class="diag-icon">3</div><div><strong>本地项目与研究包</strong><p>优先使用 OPFS 保存项目；导出包含配置、种子、时间序列、聚合统计、诊断和模型卡。</p></div></div></div></section><section class="panel mt-14"><div class="panel-head"><div><h3>自动诊断规则</h3><p>Guardrails</p></div></div><div class="panel-body diagnostic-list"><div class="diagnostic-item"><div class="diag-icon">✓</div><div><strong>状态范围检查</strong><p>概率、抑制、耗竭、氧和浸润均限制在定义域。</p></div></div><div class="diagnostic-item"><div class="diag-icon">✓</div><div><strong>配对种子比较</strong><p>相同 replicate index 共享可追溯种子生成规则，降低比较噪声。</p></div></div><div class="diagnostic-item"><div class="diag-icon">!</div><div><strong>模型边界提示</strong><p>所有药物强度均为无量纲抽象作用，不映射真实剂量、毒性或患者疗效。</p></div></div></div></section></div></div><div class="callout warning mt-14">本版本是研究与教学用机制模型。它不是患者数字孪生，不执行诊断、患者分层、真实药代动力学、毒性预测或治疗推荐。</div>`; this.byId('exportModelCardBtn').onclick = () => CRC3.downloadText('CRC-ImmunoLab-v3-model-card.md', this.modelCard(), 'text/markdown;charset=utf-8'); this.byId('exportProtocolBtn').onclick = () => CRC3.downloadText('CRC-ImmunoLab-v3-protocol.json', JSON.stringify({ applicationVersion: CRC3.VERSION, modelVersion: CRC3.BATCH_MODEL_VERSION, modelVersions: { batch: CRC3.BATCH_MODEL_VERSION, spatial: CRC3.SPATIAL_MODEL_VERSION }, config: this.project.config, parameterRegistry: CRC3.PARAMETER_EVIDENCE }, null, 2), 'application/json'); }
        showProjectModal() {
        ...
        exportResearchBundle() { const bundle = { format: 'CRC-ImmunoLab Research Bundle', formatVersion: '1.0', exportedAt: new Date().toISOString(), project: this.project, modelCard: this.modelCard(), parameterRegistry: CRC3.PARAMETER_EVIDENCE, provenance: { application: 'CRC ImmunoLab', applicationVersion: CRC3.VERSION, modelVersion: CRC3.BATCH_MODEL_VERSION, modelVersions: { batch: CRC3.BATCH_MODEL_VERSION, spatial: CRC3.SPATIAL_MODEL_VERSION }, configFingerprintScope: 'current-project-config; completed studies keep their own provenance', configFingerprint: CRC3.fingerprint(this.project.config), storageMode: this.storageMode, notice: 'Research and educational mechanism model; not for clinical decision-making.' } }; CRC3.downloadText(`${this.safeName(this.project.name)}-research-bundle.crcstudy`, JSON.stringify(bundle, null, 2), 'application/json'); this.toast('研究包已导出', '包含项目、运行、种子、统计、参数证据与模型卡。'); }
        exportStudyCsv(study) {
            const header = ['study_id', 'study_name', 'model_version', 'arm_id', 'arm_name', 'replicate', 'seed', 'scan_value', 'response', 'endpoint_tumor', 'day', 'tumor', 'cytotoxic', 'infiltration', 'exhaustion', 'hypoxia', 'suppression', 'drug', 'application_version', 'config_fingerprint'];
            const rows = [header.join(',')];
            for (const run of study.runs) {
                for (const point of run.timeSeries) {
                    rows.push([study.id, study.name, study.modelVersion, run.armId, run.armName, run.replicate, run.seed, run.scanValue ?? '', run.response, run.endpointTumor, point.day, point.tumor, point.cytotoxic, point.infiltration, point.exhaustion, point.hypoxia, point.suppression, point.drug, study.applicationVersion || 'unknown', study.configFingerprint || CRC3.fingerprint(study.config)].map(CRC3.csvEscape).join(','));
                }
            }
