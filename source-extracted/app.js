...
(function (CRC3) {
    class App {
        ...
        exportResearchBundle() { const bundle = { format: 'CRC-ImmunoLab Research Bundle', formatVersion: '1.0', exportedAt: new Date().toISOString(), project: this.project, modelCard: this.modelCard(), parameterRegistry: CRC3.PARAMETER_EVIDENCE, provenance: { application: 'CRC ImmunoLab 1', modelVersion: CRC3.VERSION, configFingerprint: CRC3.fingerprint(this.project.config), storageMode: this.storageMode, notice: 'Research and educational mechanism model; not for clinical decision-making.' } }; CRC3.downloadText(`${this.safeName(this.project.name)}-research-bundle.crcstudy`, JSON.stringify(bundle, null, 2), 'application/json'); this.toast('研究包已导出', '包含项目、运行、种子、统计、参数证据与模型卡。'); }
