...
**CRC ImmunoLab** 是一个纯前端、本地优先的肿瘤免疫空间计算研究工具，用于结直肠癌治疗方案的机制探索、批量实验设计与不确定性分析。当前版本修正了随机种子配对、第 0 天给药时间轴、导入安全与项目恢复等问题。
...
ARM_COLORS 的视觉调整与空间舞台样式必须同步到 index.html 和 source-extracted 对应文件；合并顶层主题变量不改变治疗配置或模拟计算。

## 问题闭环维护

紧凑布局断点与 1120px 最小工作台宽度对齐；验收包含 1051、1080、1119、1121px。index.html 与 source-extracted/styles.css 的对应媒体查询保持一致。
